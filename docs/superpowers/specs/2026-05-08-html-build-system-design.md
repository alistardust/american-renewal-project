...
## Review fixes (apply before implementation)

The following changes are required based on engineering review (2026-05-10):

### Section 1 — Base template

1. **Favicon:** Replace `<link rel="icon" href="{{ base }}favicon.ico">` with `<link rel="icon" type="image/svg+xml" href="{{ base }}assets/img/logo.svg">`. No `favicon.ico` exists; every current page uses the SVG logo.

2. **Autoescape:** Remove `autoescape: false` from the build script Nunjucks config. Nunjucks `{% block %}` content from child templates is never auto-escaped — only `{{ variable }}` expressions are. Setting `autoescape: false` disables escaping for `item.label`, `item.href`, `body_class`, and future variables. The correct config is:
   ```js
   const env = new nunjucks.Environment(
     new nunjucks.FileSystemLoader(['src/templates', 'src/pages'])
   );
...
3. **`{% block scripts_extra %}`:** Add `{% block scripts_extra %}{% endblock %}` immediately before `</body>` in `_base.njk`. The migration script must move any non-sitewide inline `<script>` blocks found during extraction into this block rather than dropping them silently.

4. **`src/pages/policyos.njk` header:** Add a generation header comment to the top of the file written by `generate-policyos.py`:
   ```nunjucks
...
5. **parse5 version:** Pin `"parse5": "^6.0.1"` (not `^7.0.0`). parse5 v7 is ESM-only; this repo is `"type": "commonjs"` and `require('parse5')` would throw `ERR_REQUIRE_ESM` at runtime.

### Section 3 — CI workflow

6. **CI freshness — orphan detection:** Add a third step to the freshness check after the existing two:
   ```bash
   node -e "
...
7. **Visual regression CI artifact:** Replace the single `if: always()` upload with two separate steps:
   ```yaml
   - uses: actions/upload-artifact@v4
     if: failure()
     with:
...
8. **Parity check scope:** Update the description of `check-parity.js` to scope comparison to the `<main id="main-content">` subtree only. Nav/footer differences are expected and intentional — checking them is noise. Add this sentence: "The parity check compares only the `<main id="main-content">` subtree. Nav, footer, head, and script differences are expected post-migration and are excluded."

9. **OG/Twitter meta extraction:** Add to the migration script's responsibilities: extract `og:title`, `og:description`, `og:url`, `twitter:title`, `twitter:description` from existing `<head>` into the corresponding `{% block %}` overrides in the generated `.njk` file, alongside title and description.

10. **Bypass scripts:** Add a note to Section 5: "The following scripts currently write complete HTML files directly to `docs/` and become bypass paths after migration. They must be updated to write `.njk` content blocks to `src/pages/` before they can be used post-migration: `new-pillar.js`, `new-compare.js`, `generate-pillar-cards.py`, `build_pillar_pages.py`. Until updated, these scripts produce pages without the canonical shell and must not be run after migration is complete."

### New tests required

11. **`tests/unit/build.test.js`:** Test `base` depth calculation for depth 0 (root), 1 (pillars/), and 2 (hypothetical 3rd level). Test `currentPage` computation.

12. **`tests/unit/check-html.test.js`:** For each of the 8 conformance assertions in `check-html.js`, one fixture test with a passing case and one with a violation. Also cover the source-level lint rule. Add a fixture test for non-empty `<main>` assertion (new requirement from review: `check-html.js` should also assert `<main id="main-content">` has at least one child element).

13. **`tests/e2e/site.spec.js`:** Add assertion that `a[aria-current="page"]` is present on the active nav item for a nav-destination page (e.g., verify `problem.html` has `a[aria-current="page"][href*="problem"]`).

### Critical gap (add to check-html.js assertions table)

14. **Non-empty main:** Add to the conformance assertions table in Section 3:

    | `<main id="main-content">` has ≥1 child element | Must be true |

    A `.njk` page that omits `{% block content %}` renders an empty `<main>` silently. The conformance check should catch this.
...
| Codex Review | `/codex review` | Independent 2nd opinion | 1 | issues_found | cross-model findings surfaced; 3 new issues merged |
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 1 | CLEAR (PLAN) | 10 issues total, 0 unresolved, 1 critical gap addressed |
| Design Review | `/plan-design-review` | UI/UX gaps | 0 | — | — |
| DX Review | `/plan-devex-review` | Developer experience gaps | 0 | — | — |

**CODEX:** parse5 CJS/ESM incompatibility, bypass scripts documentation, scripts_extra block requirement; all incorporated.
**CROSS-MODEL:** Both reviewers agreed on autoescape, favicon, CI artifact split, orphan detection.
**UNRESOLVED:** 0
**VERDICT:** ENG CLEARED — ready to implement with review fixes applied.
